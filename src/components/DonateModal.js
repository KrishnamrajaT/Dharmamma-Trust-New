...
const DonateModal = ({ open, onClose, amount: initialAmount, donorName: initialName, donorEmail: initialEmail, donorPhone: initialPhone }) => {
  ...
  const validate = () => {
    ...
    if (form.pan) {
      ...
      if (!/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(pan)) e.pan = 'Enter a valid PAN (e.g. ABCDE1234F)';
    }
    setErrors(e);
  ...
  const handlePaymentClick = async () => {
    ...
    try {
      ...
      // build options
      const options = {
        key: process.env.REACT_APP_RAZORPAY_KEY_ID || window.RAZORPAY_KEY_ID || 'rzp_test_XXXXXXXX',
        ...
        handler: async (response) => {
          try {
            ...
            if (verifyResult && verifyResult.success) {
              alert('Payment successful and verified. Thank you for your donation!');
            } else {
              alert('Payment processed but verification failed. We will contact you.');
            }
          } catch (err) {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Box component="form" noValidate onSubmit={(e) => { e.preventDefault(); if (tabValue === 0) handlePaymentClick(); else onClose(); }}>
          <Grid container spacing={2} sx={{ mb: 2 }}>
            <Grid item xs={12} sm={6}>
              <TextField label="Full Name" fullWidth value={form.name} onChange={handleChange('name')} error={!!errors.name} helperText={errors.name} required />
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField label="Email" fullWidth value={form.email} onChange={handleChange('email')} error={!!errors.email} helperText={errors.email} required />
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField label="Phone" fullWidth value={form.phone} onChange={handleChange('phone')} />
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField label="PAN (optional)" fullWidth value={form.pan} onChange={(e) => setForm(prev => ({ ...prev, pan: e.target.value.toUpperCase() }))} error={!!errors.pan} helperText={errors.pan} />
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField label="Amount (INR)" type="number" fullWidth value={form.amount} onChange={handleChange('amount')} error={!!errors.amount} helperText={errors.amount} required />
            </Grid>
          </Grid>
          ...
          {tabValue === 0 && (
            <Box sx={{ textAlign: 'center', py: 2 }}>
              <Typography variant="body1" sx={{ mb: 2 }}>You will be redirected to a secure payment gateway.</Typography>
              <Button variant="contained" color="primary" size="large" onClick={handlePaymentClick} disabled={loading}>
          ...
          {tabValue === 1 && (
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, gap: 3, alignItems: 'center', justifyContent: 'center', py: 1 }}>
              ...
              <Box sx={{ flex: 1 }}>
                ...
                <Box sx={{ mt: 2 }}>
                  <Typography variant="body2" sx={{ mb: 1 }}>After transfer, please email contact@dharmamma.org with your transaction details to receive a receipt.</Typography>
                </Box>
              </Box>
            </Box>
          )}
